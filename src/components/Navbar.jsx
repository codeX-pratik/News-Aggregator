...
const Navbar = ({ onSearch }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 shadow-sm transition-all duration-300 glass border-b border-white/20 dark:border-gray-700/50`}>
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <Link to="/" className="text-2xl font-bold tracking-tight flex items-center gap-2 group">
            <span className="text-blue-600 text-3xl transition-transform group-hover:scale-110">📰</span>
            <span className="text-gray-900 dark:text-white">NewsHub</span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-4 flex-1 justify-end">
            ...
            <button
              onClick={toggleTheme}
              className={`p-2 rounded-full transition-all duration-300 ${theme === 'dark' ? 'hover:bg-gray-700 text-yellow-400' : 'hover:bg-gray-200 text-gray-900'}`}
          ...
          <div className="md:hidden flex items-center gap-2">
            <button
              onClick={toggleTheme}
              className={`p-2 rounded-full ${theme === 'dark' ? 'text-yellow-400' : 'text-gray-900'}`}
            >
              {theme === 'light' ? (
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" />
